...
CouchlikeViews.prototype.getByView = function(designDocId, viewName, params, enumeration, callback) {
	...
	var methods = {
		...
		couchbaseSyncGateway: function() {
			if (!that.couchlike.engine.couchbase) { return callback(new Error('Cannot use views on couchbaseSyncGateway configuration with no couchbase config')); }
			var useView = couchbaseViewWithParams(that.couchlike.engine.couchbase, designDocId, viewName, params);
			queryCouchbaseView(that.couchlike.engine.couchbase, useView, enumeration, callback);
		}
	};
...
CouchlikeViews.prototype.getDesignDoc = function(designDocId, callback) {
	...
	var methods = {
		...
		couchbaseSyncGateway: function() {
			if (!that.couchlike.engine.couchbase) { return callback(new Error('Cannot use views on couchbaseSyncGateway configuration with no couchbase config')); }
			couchbaseGetDesignDoc(that.couchlike.engine.couchbase, designDocId, callback);
		}
	};
	methods[this.couchlike.config.couchlike.type]();
};
...
CouchlikeViews.prototype.setDesignDoc = function(designDoc, callback) {
	...
	var methods = {
		...
		couchbaseSyncGateway: function() {
			if (!that.couchlike.engine.couchbase) { return callback(new Error('Cannot use views on couchbaseSyncGateway configuration with no couchbase config')); }
			couchbaseSetDesignDoc(that.couchlike.engine.couchbase, designDoc, callback);
		}
	};
	methods[this.couchlike.config.couchlike.type]();
};
