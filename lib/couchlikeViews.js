...
function queryCouchDBView (couchlike, designDocId, viewName, params, enumeration, callback) {
	...
	couchlike.engine.bucket.view(designDocId, viewName, params, function(err, body) {
		if (err) { return callbackOrThrowError(err, callback); }

		var respondFromRows = function(rowsObject) {
			var response = [];
			if (rowsObject && rowsObject.rows && rowsObject.rows.forEach) {
				rowsObject.rows.forEach(function(row) {
					if (row && row.doc) {
		...
		if (couchlike.capability.viewIncludeDocs) {
			respondFromRows(body);
		} else {
			if (body.rows && body.rows.forEach) {
				if (USE_BULK_OPERATIONS) {
					...
					couchlike.bulkGet(retrieveIds, function(err, result) {
						if (err) { return callbackOrThrowError(err, callback); }
						respondFromRows(result);
					});
				} else {
					var retrieveHandlers = [];

					var addHandler = function(row) {
						if (row && row.id && row.id.indexOf('_sync') < 0) {
							retrieveHandlers.push(function(callback) {
								couchlike.get(row.id, function(err, document) {
									if (enumeration) { enumeration(document); }
					...
					body.rows.forEach(function(row) { addHandler(row); });

					async.parallelLimit(retrieveHandlers, 100, function(err, results) {
...
function queryCouchbaseSyncGatewayView (couchlike, designDocId, viewName, params, enumeration, callback) {
	queryCouchDBView(couchlike, designDocId, viewName, params, enumeration, callback);
}
...
function updateViewInDesignDoc(designDoc, viewName, map) {
	...
	if (JSON.stringify(designDoc.views[viewName]) !== JSON.stringify(view)) { // For Sync Gateway views, this really ought to be more sophisticated, since sync gateway injects the _sync doc removal code, this will never match.
...
CouchlikeViews.prototype.set = function(designDocId, viewName, map, callback) {
	...
	that.couchlike.views.getDesignDoc(designDocId, function(err, result){
		...
		if (updateViewInDesignDoc(result, viewName, map)) {
