...
Couchlike.prototype.get = function(id, database, callback) {
	...
	var methods = {
		...
		couchDB: function() {
			var db = that.engine.use(database);
			db.get(id, parseResult);
		},
		pouchDB: function() {
			that.engine.get(id, parseResult);
		}
...
Couchlike.prototype.set = function(id, document, database, callback) {
	function parseResult(err, result) {
	...
	var methods = {
		mock: function() {
			that.engine.set(id, document, parseResult);
		},
		couchbase: function() {
			that.engine.set(id, document, parseResult);
		},
		couchDB: function() {
			var db = that.engine.use(database);
			db.insert(document, id, parseResult);
		},
		pouchDB: function() {
			if (id) { document._id = id; }
...
Couchlike.prototype.remove = function(id, database, callback) {
	function parseResult(err, result) {
		if (callback) { callback(err, result); }
	}
	...
	var methods = {
		...
		couchDB: function() {
			...
			db.get(id, function(err, result){
				...
				else { db.destroy(result._id, result._rev, parseResult); }
			});
		},
		pouchDB: function() {
