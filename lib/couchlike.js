	...
	function parseResult(err, result) {
		...
		if (err) {
			if (callback) {
				...
				if (useStatusCode === 500 && err.reason && err.reason.indexOf && err.reason.indexOf('Internal error: Error reading view: 404 Object Not Found') === 0 ) { return callback(null, null); }
				callback(err);
			} else { throw err; }
		} else {
